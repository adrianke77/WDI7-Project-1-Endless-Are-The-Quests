...
Board.prototype.fillStartingBoard = function ( gametype ) {
  //makes every Piece instance by calling MakePiece individually for each 
  //piece and storing the instances in .layout
  //TESTED
  var pieceList = [];
  if ( gametype === "randomAll" || gametype === "randomPieces" ) {
    var pieceList = [];
    for ( var i = 0; i < 15; i++ ) {
      var pieceType = pieceTypes[ Math.floor( Math.random() * 5 ) ];
      pieceList.push( pieceType )
    }
    var kingPosition = Math.floor( Math.random() * 15 )
    pieceList.splice( kingPosition, 0, "K" ) // add only one King per side
  }
  if ( gametype === "randomPositions" ) {
    var pawnList = [ "P", "P", "P", "P", "P", "P", "P", "P" ]
    pieceList = pawnList.concat( backRowOrder )
  }
  if ( gametype === "randomAll" || gametype === "randomPositions" )
    for ( var i = 0; i < 16; i++ ) {
      this.addRandomPiecePair( pieceList[ i ] )
    }
  if ( gametype === "randomPieces" ) {
    for ( var i = 0; i < 8; i++ ) {
      this.layout[ 0 ][ i ] = this.makePiece( pieceList[ i ], "B" )
      this.layout[ 7 ][ i ] = this.makePiece( pieceList[ i ], "W" )
    }
    for ( var i = 8; i < 16; i++ ) {
      this.layout[ 1 ][ i - 8 ] = this.makePiece( pieceList[ i ], "B" )
      this.layout[ 6 ][ i - 8 ] = this.makePiece( pieceList[ i ], "W" )
    }
  }
  if ( gametype === "normal" ) {
...
Board.prototype.addRandomPiecePair = function ( type ) {
  // adds a piece for each color to the board, random position
  x = Math.floor( Math.random() * 8 );
  y = Math.floor( Math.random() * 8 );
  if ( !this.layout[ y ][ x ] ) {
    this.layout[ y ][ x ] = this.makePiece( type, "W" )
    this.layout[ 7 - y ][ x ] = this.makePiece( type, "B" )
  } else this.addRandomPiecePair( type )
